...
\setlength{\parskip}{0.25\baselineskip plus 0.25\baselineskip minus 0.25\baselineskip}
...
\makeatletter\newcommand{\ProcessResumeOptions}[1]{
    \pgfkeys{
        ...
        /resume/link/.initial={},
        /resume/email/.initial={},
        /resume/phone/.initial={},
        /resume/website/.initial={},
    }
    \pgfkeys{/resume,#1}
    \def\@thename{\pgfkeysvalueof{/resume/name}}
    \def\@theposition{\pgfkeysvalueof{/resume/position}}
    ...
    \def\@theemail{\pgfkeysvalueof{/resume/email}}
    \def\@thephone{\pgfkeysvalueof{/resume/phone}}
    \def\@thewebsite{\pgfkeysvalueof{/resume/website}}
    ...
    %
    \def\resume@name{\@thename}
    \def\resume@position{\@theposition}
    \def\resume@company{\@thecompany}
    \def\resume@location{\@thelocation}
    \def\resume@date{\@thedate}
    \def\resume@link{\@linktext}
    \def\resume@email{\@theemail}
    \def\resume@phone{\@thephone}
    \def\resume@website{\@thewebsite}
...
\makeatletter\NewDocumentCommand{\ResumeSection}{m}{
    \vspace{2\parskip}\goodbreak
    \pdfbookmark[1]{#1}{#1}
    {\Large\bfseries\scshape #1}
    \vspace{0.2\baselineskip}\hrule\vspace{0.5\baselineskip}
}\makeatother
...
%
\makeatletter\NewDocumentCommand{\ResumeHistory}{mO{}}{\parbox{\linewidth}{
...
% [#2]: The list of key=value options.
%
\makeatletter\NewDocumentCommand{\ResumeRole}{mO{}}{\parbox{\linewidth}{
    \ProcessResumeOptions{#2}
    \pdfbookmark[3]{#1}{#1}
...
% [#2]: The list of key=value options.
%
\makeatletter\NewDocumentCommand{\ResumeThesis}{mmO{}}{\vspace{\parskip}\parbox{\linewidth}{
...
\makeatletter\NewDocumentCommand{\ResumeReferee}{O{}}{\vspace{\parskip}\parbox{\linewidth}{
    \ProcessResumeOptions{#1}
    {\bfseries \resume@name}
    \if\empty\resume@position\else\\\resume@position\fi
    \if\empty\resume@email\else\\\makebox[3ex][c]{\faEnvelope[regular]} \resume@email\fi
    \if\empty\resume@phone\else\\\makebox[3ex][c]{\faMobile*} \resume@phone\fi
    \if\empty\resume@website\else\\\makebox[3ex][c]{\faExternalLink*} \resume@website\fi
}}\makeatother
